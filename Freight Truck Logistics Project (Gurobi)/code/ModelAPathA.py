...
def build_model(K, A, N, K_Q, b_kn, m, t): # Making the Model
    ...
    t_a = model.addVars(t, vtype = GRB.CONTINUOUS, lb = 0, name = 't_a') # Decision Variable representing the number of trucks 't' on arc a
    c_ak = model.addVars(K, A, vtype = GRB.CONTINUOUS, name = 'c_ak') # Binary Decision Variable representing whether commodity k is sent on arc a
